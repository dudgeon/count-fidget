#!/usr/bin/env python3
"""Instruction-level emulation of the actual Q4 STM32L072 firmware image.

Executes the committed ELF with Unicorn (QEMU Thumb core) and explicit models of
the MCU peripherals the firmware touches (RCC, PWR, FLASH, GPIO, EXTI, SYSCFG,
...
(firmware sets FLASH_ACR.LATENCY=1 without prefetch; default +0.5 cycle per
flash-resident instruction).  SPI shifting, ADC conversion, SysTick and the
TPS22917 turn-on (typical tON for 22 nF CT) are event-timed.  Analog behaviour
(rail droop, currents, charge-pump output, protector state) is not simulated.
Results are "reproduced in a model", never "observed on hardware".
"""
import argparse
import json
import random
...
class Oled:
    ...
    def update_power(self):
        """U6 on when PB1 drives high. Rise ~tON 84 ms (typ, 22 nF CT), fall via QOD."""
    ...
    def tick(self):
        if self.switch_on_at is not None and not self.powered:
            if self.sim.now_s() - self.switch_on_at >= 0.084:   # TPS22917 tON typ at CT=22nF
                self.powered = True
...
class Sim:
    def __init__(self, elf_path, optr=0x807C00AA, vlogic=3.20, fram_fill=0x00, seed=1, flash_ws_penalty=0.5):
    ...
    def map_memory(self):
        ...
        uc.mem_map(0x1FF80000, 0x1000)
        uc.mem_write(0x1FF80000, bytes(sysmem))
    ...
    def oled_reset_pins(self):
        # all GPIO return to reset (analog) state while NRST is low
    ...
    def gpio_level(self, port, pin):
        ...
        if port == 'A' and pin == 0: return 0 if self.sw1 else 1          # R17 pull-up, SW1 to GND
    ...
    def pin_levels(self):
        return {(p, n): self.gpio_level(p, n) for p, n in (('A', 0), ('A', 1), ('B', 0), ('B', 1), ('B', 8), ('B', 12), ('B', 14))}
    ...
    def service_timers(self):
        ...
        if self.oled.switch_on_at is not None and not self.oled.powered:
            nxt.append((self.oled.switch_on_at + 0.084) * HZ)
    ...
    def run(self, until_s):
        ...
        self.service_timers()
        if getattr(self, 'wfi_pending', None) is not None:
    ...
    def ahb_read(self, uc, off, size, ud):
        ...
        if 0x40022000 <= a < 0x40022400:
            ...
            if o == 0x18: return bit(3)            # READY, not busy
...
def decode_display(oled):
    ...
    for x in range(58, 69):
        ...
        label.append(v)
    status = 'ERR' if label == ERR else 'MAX' if label == MAX else ('' if not any(label) else '??')
...
def summary(sim, label):
    ...
    return dict(label=label, t=round(sim.now_s(), 4), halted=sim.halted,
                ...
                boots=sim.boot_count, dfu=sim.dfu,
