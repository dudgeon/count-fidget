...
## Latest verified website state — 15 September 2026, 10:36 UTC
...
- **PCBWay counters at 10:36 UTC:** **Under Review 2, Awaiting Payment 0, Production 0**. Existing Q1 references remain W914112AS1N4 / T-1N5W914112A; no Q2 file submission, payment or manufacturing release.
...
- **Q2 email approval remains pending:** automatic approval review rejected the specific pad-pattern/file-replacement email; it has **not been sent**. Do not retry by another channel. PCBWay's 03:23:59 UTC request for **email-only** follow-up remains in effect; no combined mask opening has been approved.
- **Engineering/package:** Q2 routing is being completed; the latest native check found one SW2 thermal-relief issue being corrected. Final checks/exports/package are pending. All vendor-uploaded files remain Q1, and all physical qualification/first-article gates remain open.
...
- **Q2 reply attempted but NOT SENT:** automatic approval review rejected the prepared pad-pattern/file-replacement inquiry, stating that revised engineering-detail disclosure requires specific user approval despite the established quote authorization. A focused approval question is pending. Keep the local draft under ignored `private/`; do not retry through another channel or imply supplier receipt. The existing Q1 engineering hold remains the last confirmed outbound state.
