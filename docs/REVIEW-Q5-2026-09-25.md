...
| Area | Finding | Fix |
|---|---|---|
| Latch | A short COUNT press could release U7 before PB2 took over; the power-on press could be lost | The reset handler drives PB2 first. C40 4.7 nF on SYS_ON bridges the gap (4.67 ms). The boot scan captures edges. R38 isolates the VBUS path. Minimum press 15.8 ms typ / 21.6 ms allowance. Emulator B5/B6 |
...
| [#12](https://github.com/dudgeon/count-fidget/issues/12) soft power | COUNT/SW1 switches SYS onto U7 ON; PB2 holds it; VBUS holds it via D4. The power-on press counts. 30 s auto-off on battery; Stop fallback with USB. | Emulator B1–B5, U1–U5, C4, H1/H2; latch press ≥ 15.8 ms typ / 21.6 ms allowance (with C40); inrush below the 24.8 mA minimum OCD over temperature | Latch timing with a bouncing contact; release behaviour |
