...
- **SW3, TS-1088-AR02016**, provides a physical reset button reached through a bottom service pinhole. Holding the reset-count key at the moment reset is released selects factory USB DFU (with a battery attached, press the pinhole reset first so the running application never sees a count-reset press; see `q4-firmware.md` step 2); no external programmer is needed for ordinary home programming. SWD test lands remain for deeper repair.
