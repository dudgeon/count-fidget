...
- **Supply voltage:** the module still runs from VLOGIC through U6, 3.151–3.259 V regulated. The module drawing gives an external supply range of 3–5 V (§2). The 3.5–4.2 V figure (§3.2) is the SSD1315 internal DC/DC VBAT specification. Which one governs depends on the module's undocumented internal power tree.
  ...
  - Feeding the module from SYS_LOAD instead was rejected for now. Its 4.5 V USB level is inside the module's VCC range but outside the SSD1315's VBAT 4.5 V abs limit if there is no internal regulator. That cannot be decided without a sample.
...
- The 3.35 V cut-off keeps the TLV767 out of dropout and the display off a collapsing rail. From cut-off, ~5 % of capacity remains before the BQ29700 disconnects at 2.8 V. At the 8 µA off current plus self-discharge, that is about **9.5 days**. After a protector disconnect, the cell recovers by charging from USB.
