"""Generate the separate Q5 native schematic from its controlled connectivity model.
...
PROJECT = "click-counter-Q5"
LIB = "CountFidgetQ5"
NS = uuid.UUID("5a1f0c22-8d3e-5e41-9b07-2c6d4e8f1a35")
...
def pins_for(p):
    ...
    if ref == 'U1':
        raise ValueError('Q5 MCU needs reviewed physical symbol_pins metadata')
    if ref in IC_PINS:
...
SHEET_INFO = {
 'mcu': ('STM32, keys and USB boot controls', 'STM32L072CBT6. USB ROM DFU uses BOOT0 held high during NRST reset. PB2 holds the soft power latch;\nPA4 reads SYS_LOAD/2; PA5/PA6 read charger status. COUNT closes SYS to PWR_KEY; Q5 mirrors it onto COUNT_N.'),
 'display': ('SPI OLED module and power control', 'Complete HS96L01W4S03 module; seven-pin SPI interface. U6 QOD is unconnected so the module supply floats when off.\nSignals must be high impedance before module power is disabled. Mechanical drawing controls the footprint.'),
 ...
 'usb-power': ('USB, charging, soft power latch and common supply', 'U7 ON = VBUS (D4) OR COUNT key (D3 A1) OR PWR_HOLD (D3 A2), pulled down by R35.\nOff: only BQ29700 and BQ25185 battery quiescent current remain. Cold cell insertion is not proven by the slew switch.'),
 'battery': ('On-board LIR2032 holder and protection', 'CELL_N_RAW is isolated from system ground by the protection path. BT1 accepts the rechargeable LIR2032 only.\nCell compatibility, protection, USB rearm and cold insertion remain qualification gates.'),
 'thermal': ('Independent charge-temperature window', 'TH1 on the board beside BT1; both window outputs must enable their series FETs for charging.\nGate pulldowns disable charging when comparator power is absent; threshold and fault tests remain required.'),
}
...
def header(identity,title):
    return f'''(kicad_sch (version 20250114) (generator "eeschema") (generator_version "10.0")
      (uuid "{identity}") (paper "A3") (title_block (title {q(title)}) (rev "Q5 candidate")
...
def main():
    ...
    (OUT/'sym-lib-table').write_text(f'(sym_lib_table (version 7) (lib (name "{LIB}")(type "KiCad")(uri "${{KIPRJMOD}}/{LIB}.kicad_sym")(options "")(descr "Q5 exact package pins; reviewed custom symbols")))\n')
    # Remove sheet files left by an earlier generation with more pages.
    current={name+'.kicad_sch' for name,_,_,_ in PAGES}|{PROJECT+'.kicad_sch'}
    for stale in OUT.glob('*.kicad_sch'):
    ...
    rootitems.insert(0,text(f'Q5 schematic index - {model["fitted_components"]} fitted PCB parts plus non-fitted test lands and mounts.\nGlobal net labels connect the functional sheets. Crosses explicitly mark unused physical pins.\nQ1 submitted files remain frozen. This candidate still requires ERC, layout, first-article and safety qualification.',20,20,1.27))
    (OUT/(PROJECT+'.kicad_sch')).write_text(header(ROOT_ID,'Count Fidget - Q5 engineering candidate')+'\n(lib_symbols)\n'+'\n'.join(rootitems)+'\n(sheet_instances (path "/" (page "1")))\n)\n')
    (OUT/'schematic-paths-Q5.json').write_text(json.dumps({'root_uuid':ROOT_ID,'namespace':str(NS),'parts':mapping},indent=2)+'\n')
    print('Generated native Q5 sheets, exact package pin symbols, and deterministic PCB/schematic paths.')
    if args.kicad_cli:
...
def validate(cli,model,mapping):
    ...
    schematic='electronics/q5/'+PROJECT+'.kicad_sch'
    commands=[
        ['sch','export','netlist','--format','kicadxml','-o','electronics/q5/schematic-netlist-Q5.xml',schematic],
        ['sch','erc','--format','json','--severity-all','--exit-code-violations','-o','electronics/q5/erc-Q5.json',schematic],
        ['sch','export','pdf','-o','electronics/q5/schematic-Q5.pdf',schematic],
        ['sch','export','svg','-o','electronics/q5/schematic-svg/',schematic],
    ]
    # CLI instances share a macOS lock; keep these sequential even though exports are independent.
    import shutil
    shutil.rmtree(OUT/'schematic-svg',ignore_errors=True)
    for command in commands:
        subprocess.run([str(cli),*command],cwd=ROOT,check=True)
    xml=OUT/'schematic-netlist-Q5.xml'; native=ET.parse(xml); tree=native.getroot()
    ...
    assert set(components)==set(parts), 'Native schematic component references differ from Q5 model'
    ...
    erc=json.loads((OUT/'erc-Q5.json').read_text())
    ...
    files=[OUT/(PROJECT+'.kicad_sch'),*(OUT/(name+'.kicad_sch') for name,_,_,_ in PAGES),
           OUT/(LIB+'.kicad_sym'),OUT/'sym-lib-table',OUT/'schematic-paths-Q5.json',
           xml,OUT/'erc-Q5.json',OUT/'schematic-Q5.pdf',*sorted((OUT/'schematic-svg').glob('*.svg'))]
    inputs=[Path(__file__).resolve(),ROOT/'scripts/build_q5_model.py',OUT/'netlist-Q5.json']
    if (OUT/'placement.json').exists(): inputs.append(OUT/'placement.json')
    report={'status':'Native Q5 schematic candidate; not a hardware release','kicad_version':erc['kicad_version'],
            ...
            'supply_assertions':{'VBUS':'External USB source','GND':'System return via reviewed power topology',
                                 'CELL_N_RAW':'External cell return','BAT_SENSE':'Cell supply through R7','SYS_LOAD':'U7 output from SYS'},
    ...
    (OUT/'schematic-validation-Q5.json').write_text(json.dumps(report,indent=2)+'\n')
