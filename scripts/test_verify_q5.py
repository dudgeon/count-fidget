"""Adversarial Q5 verifier checks on temporary copies of a valid native baseline.

Default requires verify_q5.saved() to pass BEFORE any corruption. --semantic-only
...
class Fixture:
    def __init__(self, root):
        self.root = root
        self.board = root / 'electronics/q5/click-counter-Q5.kicad_pcb'
        self.xml = root / 'electronics/q5/schematic-netlist-Q5.xml'
        self.report = root / 'verification/q5-validation.json'
        self.touched = {}
    ...
    def run(self, mode):
        if mode in ('semantic', 'bound'):
            command = [sys.executable, '-B', 'scripts/verify_q5.py']
            if mode == 'semantic':
            ...
            command = [sys.executable, '-B', '-c',
                "import sys;sys.path.insert(0,'scripts');import verify_q5 as v;"
...
def coherent_mutation(fixture, *, wrong_pin=False):
    ...
    """
    source = Path('scripts/build_q5_model.py')
    ...
    model_path = Path('electronics/q5/netlist-Q5.json')
    ...
    if wrong_pin:
        fixture.json('electronics/q5/schematic-paths-Q5.json',
...
def main():
    ...
    if not args.semantic_only:
        ...
        originals.add('verification/q5-validation.json')
    passed = []
    with tempfile.TemporaryDirectory(prefix='count-fidget-q5-negative-') as directory:
        ...
        if not args.semantic_only:
            ...
            for label, name, expected in [
                ('Bound source modified', 'scripts/build_q5_board.py', 'Stale native evidence'),
                ('Bound local footprint modified', next(n for n in hashes if n.endswith('.kicad_mod')), 'Stale native evidence'),
                ('Bound circuit contract modified', 'electronics/q5/circuit-contract.json', 'Stale native evidence'),
                ('Bound ERC bytes modified', 'verification/q5-native-erc.json', 'Native reports changed'),
                ('Bound DRC bytes modified', 'verification/q5-native-drc.json', 'Native reports changed'),
                ('Bound native XML bytes modified', 'verification/q5-native-netlist.xml', 'Native reports changed')]:
                rejects(label, lambda n=name: append(n), expected, 'bound')
            def bound(name, mutate, expected):
                rejects(name, lambda: f.json('verification/q5-validation.json', mutate), expected, 'bound')
            bound('Bound input digest changed', lambda r: r['inputs_sha256'].__setitem__('scripts/build_q5_model.py', '0'*64), 'Stale native evidence')
            bound('Bound report digest changed', lambda r: r['reports_sha256'].__setitem__('verification/q5-native-drc.json', '0'*64), 'Native reports changed')
            ...
            def native(name, kind, mutate, expected):
                rejects(name, lambda: f.json('verification/q5-native-'+kind+'.json', mutate), expected, 'report-'+kind)
