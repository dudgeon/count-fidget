"""Reproducible Q4 STM32L072CBT6 build; EEPROM and option bytes never loaded."""
import argparse
import hashlib
import json
import re
...
import subprocess
import tempfile
from test_firmware_q4 import run_tests
ROOT=Path(__file__).resolve().parents[1]
SOURCE=ROOT/'firmware/q4-stm32'
STEM='count-fidget-Q4-stm32'
SOURCES=('startup.c','main_stm32.c','runtime.c','counter.c','input.c','journal.c','app.c','fram.c','oled.c','rail.c')
...
def inputs():
    ...
    files += [ROOT/'scripts/build_firmware_q4.py',ROOT/'scripts/test_firmware_q4.py']
...
def validate(directory):
    ...
    vectors={'Reset_Handler':1,'SysTick_Handler':15,'EXTI0_1_IRQHandler':21,'I2C1_IRQHandler':39,'SPI2_IRQHandler':42}
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--toolchain',type=Path);p.add_argument('--output',type=Path,default=SOURCE/'build');p.add_argument('--verify-only',action='store_true');p.add_argument('--eeprom-alternate',action='store_true')
    a=p.parse_args();output=a.output.resolve()
    if ROOT in output.parents and SOURCE not in output.parents:p.error('Repository output must be in firmware/q4-stm32')
    if a.verify_only:verify_manifest(output);print('PASS: Q4 source/output hashes and target ELF/HEX/BIN/vector/load checks');return
    ...
    with tempfile.TemporaryDirectory(prefix='count-fidget-q4-build-') as temporary:
        stage=Path(temporary)
        for name in snapshot:
            if name.startswith('firmware/q4-stm32/'):
                relative=Path(name).relative_to('firmware/q4-stm32');dest=stage/relative;dest.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(ROOT/name,dest)
        ...
        if a.eeprom_alternate:flags+=['-DBOARD_STORAGE_FRAM=0']
        ...
        m={'schema':1,'status':'Q4 engineering target; physical qualification pending','target':'STM32L072CBT6','storage':'EEPROM Bank2 alternate' if a.eeprom_alternate else 'FM25V02A-GTR SPI1 FRAM','display':'HS96L01W4S03 / SSD1315 SPI2','inputs':snapshot,'outputs':{n:record(stage/n) for n in OUTPUTS},'validation':validation,'dependencies':dependencies,'commands':commands,'size':size,'compiler':{'version':version,'tools':{n:record(t) for n,t in tool.items()},'package_url':'https://developer.arm.com/-/media/Files/downloads/gnu/14.3.rel1/binrel/arm-gnu-toolchain-14.3.rel1-darwin-arm64-arm-none-eabi.tar.xz','archive_sha256':'30f4d08b219190a37cded6aa796f4549504902c53cfc3c7e044a8490b6eba1f7'}}
    ...
    verify_manifest(output);print(size);print('PASS: Q4 target compiled, host fault models passed, Bank1-only ELF/HEX/BIN verified')
