"""Build the separate two-layer Q4 placement candidate using exact local lands.

Run with KiCad's pcbnew Python. This intentionally clears Q4 routing when
...
def main():
    ...
    if path.exists() and not args.replace_candidate:
        ap.error('Q4 exists: explicit --replace-candidate required to clear its routing')
    ...
    for part in model['parts']:
        ...
        source=ROOT/'electronics/q3/CountFidgetQ3.pretty'/(name+'.kicad_mod')
        if source.exists():shutil.copyfile(source,target)
        else:raise FileNotFoundError('Exact reviewed footprint required: '+str(target))
    board=p.LoadBoard(str(ROOT/'electronics/q3/click-counter-Q3.kicad_pcb'))
    discarded=[]
    ...
    records=json.loads((OUT/'schematic-paths-Q4.json').read_text())['parts']
    for part in model['parts']:
    ...
    for item in board.GetDrawings():
        if isinstance(item,p.PCB_TEXT):
            item.SetText(item.GetText().replace('Q3','Q4'))
    pro=json.loads((ROOT/'electronics/q3/click-counter-Q3.kicad_pro').read_text())
    pro['meta']['filename']='click-counter-Q4.kicad_pro'
    pro['board']['design_settings']['drc_exclusions']=[]
    (OUT/'click-counter-Q4.kicad_pro').write_text(json.dumps(pro,indent=2)+'\n')
    (OUT/'fp-lib-table').write_text('(fp_lib_table (version 7) (lib (name "CountFidgetQ4") (type "KiCad") (uri "${KIPRJMOD}/CountFidgetQ4.pretty") (options "") (descr "Q4 exact reviewed lands; assembler acceptance pending")))\n')
    ...
    assert p.ExportSpecctraDSN(board,str(OUT/'click-counter-Q4.dsn'))
    (OUT/'netlist-Q4.json').write_text(json.dumps(model,indent=2)+'\n')
    print('Q4 two-layer placement candidate:',model['fitted_components'],'fitted parts; no routing/physical qualification claimed.')
