"""Generate the separate Q2 native schematic from its controlled connectivity model.
...
PROJECT = "click-counter-Q2"
LIB = "CountFidgetQ2"
NS = uuid.UUID("ca545a54-6b1d-5c8d-850d-39f1f533985e")
...
def pins_for(p):
    ...
    if ref == 'DS1':
        commons={'20':'COM1','1':'COM2','11':'COM3','10':'COM4'}
        return [(str(i),commons.get(str(i),'SEGMENT'),'passive') for i in range(1,21)]
...
PAGES = [
 ('mcu-lcd','MCU, LCD bias and programming',[
  ('U1',105,125),('DS1',290,92),('C8',260,168),('C14',230,202),('C15',290,202),('C16',350,202),
  ('C7',230,235),('R16',290,235),('C10',350,245),('TP1',70,225),('TP2',145,225),('TP3',70,250),('TP4',145,250)],
  '48-pin DGG device. All unused GPIO/crystal pins explicitly NC. Q2 firmware: 32 Hz nominal.\nMode 2 requires C8 pump plus C14/C15/C16 bias reservoirs. Hardware waveform/FRAM tests remain open.'),
 ('usb-power','USB input, charging and 3 V supply',[
  ('J1',75,85),('D1',200,66),('U2',200,135),('U3',335,135),('R1',50,159),('R2',112,159),
  ('C9',50,194),('C1',112,194),('R3',184,194),('R4',249,194),('R5',315,194),
  ('R6',50,226),('R19',112,226),('C13',184,226),('C2',249,226),('C3',315,226),
  ('C5',249,255),('C6',365,170),('TP5',55,255),('TP6',120,255)],
  'USB is charge-only: D+/D-/SBU pins explicitly NC. STAT1/STAT2 unused open-drain outputs are NC.\nTPS7A0230PDBVR includes active output discharge. Pin 4 may be grounded. No manufacture release.'),
 ('battery-protection','Battery connector and independent cell protection',[
  ('J2',70,92),('U4',210,92),('Q1',110,160),('Q2',230,160),('R7',320,80),('C4',320,112),
  ('R8',320,164),('R9',320,196),('TP7',100,222),('TP8',220,222),('H1',70,257),('H2',140,257)],
  'J2 mates the offboard CC-BAT-001 cell/NTC harness. CELL_N_RAW must remain isolated from system GND.\nBQ29700 undervoltage tolerance, protection bypass/recovery and final pack remain release gates.'),
 ('thermal','Independent temperature window and charge inhibit',[
  ('U5',190,105),('R10',70,66),('R11',70,105),('R12',70,144),('R13',70,183),
  ('Q3',315,92),('Q4',315,158),('R14',195,194),('R15',315,216)],
  'TLV7032 push-pull outputs remain separate. Both series FETs must conduct to pull CE_N low.\nGate pulldowns hold charging disabled after comparator power loss. POR, threshold and fail-safe tests remain open.'),
 ('buttons','Count/reset keys, filtering and fixture access',[
  ('SW1',105,95),('SW2',285,95),('R17',105,150),('R18',285,150),('C11',105,200),('C12',285,200),
  ('TP9',105,245),('TP10',285,245)],
  'Cherry MX1A-E1NW, 19.05 mm stem spacing. COUNT and RESET are active low, with 470k pull-ups.\nFirmware captures one count per accepted press; physical switch/clip/plate support remains unqualified.'),
]


def header(identity,title):
    return f'''(kicad_sch (version 20250114) (generator "eeschema") (generator_version "10.0")
      (uuid "{identity}") (paper "A3") (title_block (title {q(title)}) (rev "Q2 candidate")
...
def main():
    ...
    (OUT/'sym-lib-table').write_text(f'(sym_lib_table (version 7) (lib (name "{LIB}")(type "KiCad")(uri "${{KIPRJMOD}}/{LIB}.kicad_sym")(options "")(descr "Q2 exact package pins; reviewed custom symbols")))\n')
    mapping={}; rootitems=[]
    for index,(name,title,positions,note) in enumerate(PAGES,2):
        ...
        flags = [('VBUS',365,60,1),('GND',365,88,2)] if name=='usb-power' else [('CELL_N_RAW',355,220,3),('BAT_SENSE',355,245,4)] if name=='battery-protection' else []
        if flags:
    ...
    rootitems.insert(0,text('Q2 schematic index - 50 fitted PCB parts plus 10 pogo lands and 2 mounts.\nGlobal net labels connect the functional sheets. Crosses explicitly mark unused physical pins.\nQ1 submitted files remain frozen. This candidate still requires ERC, layout, first-article and safety qualification.',20,20,1.27))
    (OUT/(PROJECT+'.kicad_sch')).write_text(header(ROOT_ID,'Count Fidget - Q2 engineering candidate')+'\n(lib_symbols)\n'+'\n'.join(rootitems)+'\n(sheet_instances (path "/" (page "1")))\n)\n')
    (OUT/'schematic-paths-Q2.json').write_text(json.dumps({'root_uuid':ROOT_ID,'namespace':str(NS),'parts':mapping},indent=2)+'\n')
    print('Generated six native sheets, exact package pin symbols, and deterministic PCB/schematic paths.')
    if args.kicad_cli:
...
def validate(cli,model,mapping):
    ...
    schematic='electronics/q2/'+PROJECT+'.kicad_sch'
    commands=[
        ['sch','export','netlist','--format','kicadxml','-o','electronics/q2/schematic-netlist-Q2.xml',schematic],
        ['sch','erc','--format','json','--severity-all','--exit-code-violations','-o','electronics/q2/erc-Q2.json',schematic],
        ['sch','export','pdf','-o','electronics/q2/schematic-Q2.pdf',schematic],
        ['sch','export','svg','-o','electronics/q2/schematic-svg/',schematic],
    ]
    ...
    xml=OUT/'schematic-netlist-Q2.xml'; native=ET.parse(xml); tree=native.getroot()
    ...
    assert set(components)==set(parts), 'Native schematic component references differ from Q2 model'
    ...
    erc=json.loads((OUT/'erc-Q2.json').read_text())
    ...
    files=[OUT/(PROJECT+'.kicad_sch'),*(OUT/(name+'.kicad_sch') for name,_,_,_ in PAGES),
           OUT/(LIB+'.kicad_sym'),OUT/'sym-lib-table',OUT/'schematic-paths-Q2.json',
           xml,OUT/'erc-Q2.json',OUT/'schematic-Q2.pdf',*sorted((OUT/'schematic-svg').glob('*.svg'))]
    inputs=[ROOT/'scripts/build_q2_schematic.py',ROOT/'scripts/build_q2_model.py',ROOT/'electronics/netlist.json']
    report={'status':'Native Q2 schematic candidate; not a hardware release','kicad_version':erc['kicad_version'],
            ...
            'supply_assertions':{'VBUS':'External USB source','GND':'System return via reviewed power topology',
                                 'CELL_N_RAW':'External cell return','BAT_SENSE':'Cell supply through R7'},
    ...
    (OUT/'schematic-validation-Q2.json').write_text(json.dumps(report,indent=2)+'\n')
