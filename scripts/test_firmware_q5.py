"""Compile/run the production Q5 portable code against deterministic fault models."""
import argparse
import hashlib
import json
import shutil
...
GROUPS={'behavior':['test_behavior.c','counter.c','input.c','journal.c','app.c'],
        'battery':['test_battery.c','battery.c'],
...
def run_tests(sanitize=False,cc='clang'):
    outputs={}
    source_files=[p for p in SOURCE.glob('*') if p.suffix in ('.c','.h')]
    before={p.name:record(p) for p in source_files}
    with tempfile.TemporaryDirectory(prefix='count-fidget-q5-tests-') as td:
        for name,files in GROUPS.items():
            exe=Path(td)/name
            command=[cc,'-std=c11','-O2','-Wall','-Wextra','-Werror','-Wno-misleading-indentation','-I'+str(SOURCE)]
...
def image_checks(directory):
    from build_firmware_q5 import validate,verify_manifest,hex_image,elf_image,validate_ram_stop,STEM,OUTPUTS
    baseline=verify_manifest(directory);rejections=[]
    with tempfile.TemporaryDirectory(prefix='q5-image-negative-') as td:
        out=Path(td)
        def reset():
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--sanitize',action='store_true');p.add_argument('--output',type=Path);p.add_argument('--image-only',action='store_true');p.add_argument('--cc',default='clang',help='host C compiler (clang or gcc)');p.add_argument('--images',type=Path,default=SOURCE/'build')
    a=p.parse_args()
    if a.image_only:
        ...
        return
    outputs=run_tests(a.sanitize,a.cc)
    result={'schema':1,'target':'STM32L072CBT6 Q5','passed':True,'sanitized':a.sanitize,'sanitizer':'UndefinedBehaviorSanitizer' if a.sanitize else None,
            ...
            'runner':record(Path(__file__)),'host_compiler':subprocess.check_output([a.cc,'--version'],text=True).splitlines()[0]}
