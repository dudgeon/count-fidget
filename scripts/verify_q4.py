"""Verify Q4 native agreement and freshly bound ERC/DRC evidence.

This checks design files and native connectivity, not physical electrical function.
No courtyard or electrical DRC exceptions are accepted in Q4.
"""
import argparse
...
Q4=ROOT/'electronics/q4'
VERIFY=ROOT/'verification'
BOARD=Q4/'click-counter-Q4.kicad_pcb'
SCHEMATIC=Q4/'click-counter-Q4.kicad_sch'
PUBLIC_XML=Q4/'schematic-netlist-Q4.xml'
NATIVE_XML=VERIFY/'q4-native-netlist.xml'
DRC=VERIFY/'q4-native-drc.json'
ERC=VERIFY/'q4-native-erc.json'
REPORT=VERIFY/'q4-validation.json'
PREPARE=VERIFY/'q4-native-preparation.json'
...
def input_hashes():
    paths={BOARD,SCHEMATIC,PUBLIC_XML,Q4/'netlist-Q4.json',Q4/'click-counter-Q4.kicad_pro',
           Q4/'schematic-paths-Q4.json',Q4/'sym-lib-table',Q4/'fp-lib-table',Path(__file__).resolve(),
           ROOT/'scripts/kicad_sexpr.py',ROOT/'scripts/build_q4_model.py',ROOT/'scripts/build_q4_schematic.py',
           ROOT/'scripts/build_q4_board.py',ROOT/'scripts/sync_q4_board.py',
           ROOT/'scripts/preroute_q4.py',ROOT/'scripts/prepare_q4_route.py',ROOT/'scripts/finish_q4_board.py',
           ROOT/'scripts/measure_q4_routes.py',
           ROOT/'electronics/q3/netlist-Q3.json'}
    for pattern in ('*.kicad_sch','*.kicad_sym','*.kicad_dru','*.pretty/*.kicad_mod'):
        paths.update(Q4.glob(pattern))
    for name in ('placement.json','circuit-contract.json','locked-route-seed.json'):
        if (Q4/name).exists(): paths.add(Q4/name)
...
def verify_semantics(xml_path=PUBLIC_XML):
    model=make_model()
    require(json.loads((Q4/'netlist-Q4.json').read_text())==model,'Saved Q4 model is stale')
    ...
    require(parts['U1']['mpn']=='STM32L072CBT6','Selected Q4 MCU differs')
    ...
    mapping=json.loads((Q4/'schematic-paths-Q4.json').read_text())
    ...
    for ref,part in parts.items():
        ...
        local=Q4/(part['footprint'].split(':')[0]+'.pretty')/(part['footprint'].split(':')[1]+'.kicad_mod')
    ...
    require(set(layers)=={'F.Cu','B.Cu'} and model['layers']==2,'Q4 must use two copper layers')
    ...
    contract=Q4/'circuit-contract.json'
    require(contract.exists(),'Reviewed Q4 circuit contract missing')
...
def report_checks(path,kind):
    ...
    if kind=='drc':
        ...
        require(len(data.get('sheets',[]))==len(list(Q4.glob('*.kicad_sch'))),'ERC sheet count differs')
...
def main():
    ...
    try:
        ...
        print('Q4 design-file checks passed; physical qualification and release remain separate.')
