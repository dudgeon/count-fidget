    ...
    if(column>=16 && column<112) {
        ...
        if(x<10 && glyphs[cell]<10) {
            ...
            for(unsigned bit=0;bit<8;bit++) {
                unsigned y=page*8U+bit;
                if(y>=8 && y<50 && (shape&(1U<<((y-8U)/6U))))result|=(uint8_t)(1U<<bit);
            }
        }
    }
    if((maximum || fault) && page==7 && column>=58 && column<69)
...
void oled_service(Oled *d,const OledHal *h,bool awake,uint32_t count,bool input_fault,uint32_t now) {
    ...
    case OLED_RESET_WAIT:
        /* Whole module stabilizes with reset held low for250ms, exceeding
         * the supplier sample's200ms reset. Then release and wait2ms. */
    ...
    case OLED_NEXT_CHUNK:
        ...
        if(d->page<8U) d->state=OLED_ADDRESS_CHUNK;
    ...
    case OLED_STOP_PUMP:
        ...
        /* The completion path starts a200 nominal ms guard after ACK. */
        break;
    case OLED_STOP_WAIT:
        if(reached(now,d->deadline)) {
