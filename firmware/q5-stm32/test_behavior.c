...
/* Hold the reset key for hold_ms (sampled every ms), then release for 20 ms. */
static void hold_reset(App*a,uint32_t*t,uint32_t hold_ms,uint8_t extra){for(uint32_t i=0;i<hold_ms;i++)sample(a,INPUT_RESET|extra,(*t)++);for(unsigned i=0;i<20;i++)sample(a,0,(*t)++);}
static unsigned long key_cases;
static void key_semantics(void){
    /* Analytic hold-duration oracle across timestamp wrap: debounced press to
     * debounced release equals the raw hold, which must lie within
     * [RESET_HOLD_MS, RESET_ABANDON_MS] for a reset; anything else leaves the
     * count untouched. RST is visible from RESET_HOLD_MS after the debounced press. */
    for(unsigned wrap=0;wrap<2;wrap++)for(uint32_t hold=0;hold<=RESET_ABANDON_MS+60;hold+=(hold<1900||hold>10100)?97:1){
        if(hold>2100&&hold<9900&&hold%250)continue;
        Counter c;uint32_t t=wrap?0xffffff00UL:0;counter_init(&c,77,t,false,false);bool armed=false;
        for(uint32_t i=0;i<hold;i++){counter_sample(&c,false,true,t++);armed|=c.reset_armed;}
        for(unsigned i=0;i<20;i++)counter_sample(&c,false,false,t++);
        bool reset=hold>=RESET_HOLD_MS&&hold<=RESET_ABANDON_MS;
        assert(c.count==(reset?0U:77U));
        assert(armed==(hold>=RESET_HOLD_MS+DEBOUNCE_MS+1U));
        assert(!c.reset_armed&&!c.reset_holding);++key_cases;
    }
    /* Increment wins: a COUNT press at any time during a qualifying hold
     * counts once and cancels the reset; simultaneous presses count. */
    for(uint32_t at=0;at<2950;at+=37){
        Counter c;uint32_t t=0;counter_init(&c,5,t,false,false);
        for(uint32_t i=0;i<3000;i++){bool inc=i>=at&&i<at+40;counter_sample(&c,inc,true,t++);}
        for(unsigned i=0;i<20;i++)counter_sample(&c,false,false,t++);
        assert(c.count==6);++key_cases;
    }
    {Counter c;counter_init(&c,5,0,false,false);uint32_t t=0;
     for(unsigned i=0;i<50;i++)counter_sample(&c,true,true,t++);
     for(unsigned i=0;i<3000;i++)counter_sample(&c,false,true,t++);
     for(unsigned i=0;i<20;i++)counter_sample(&c,false,false,t++);
     assert(c.count==6);} /* pressed together: increment, and the hold never resets */
    /* Keys already held at initialization (MCU reset, DFU attempt, wake key)
     * are levels, not presses: no count, and a later release never resets. */
    {Counter c;counter_init(&c,5,0,false,true);uint32_t t=0;
     for(unsigned i=0;i<5000;i++)counter_sample(&c,false,true,t++);
     for(unsigned i=0;i<20;i++)counter_sample(&c,false,false,t++);assert(c.count==5);
     counter_init(&c,5,0,true,false);t=0;for(unsigned i=0;i<100;i++)counter_sample(&c,true,false,t++);assert(c.count==5);
     for(unsigned i=0;i<20;i++)counter_sample(&c,false,false,t++);for(unsigned i=0;i<20;i++)counter_sample(&c,true,false,t++);assert(c.count==6);
     counter_init(&c,5,0,true,false);assert(counter_wake_press(&c,0)==COUNTER_EVENT_INCREMENT&&c.count==6);
     for(unsigned i=0;i<200;i++)counter_sample(&c,true,false,i);for(unsigned i=200;i<220;i++)counter_sample(&c,false,false,i);assert(c.count==6);}
    /* Stuck reset key sleeps normally after being abandoned. */
    {Counter c;counter_init(&c,9,0,false,false);uint32_t t=0;for(unsigned i=0;i<45000;i++)counter_sample(&c,false,true,t++);
     assert(!c.awake&&c.count==9&&!c.reset_armed);for(unsigned i=0;i<20;i++)counter_sample(&c,false,false,t++);assert(c.count==9);}
}
static void behavior(void){
    ...
    for(unsigned offset=0;offset<17;offset++)for(unsigned high=1;high<=32;high++)for(unsigned stall=1;stall<=13;stall++){
        Counter c;uint32_t t=0xfffffff0UL+offset;counter_init(&c,17,t,false,false);InputQueue q={0};InputState state={0};
    ...
    Counter c;counter_init(&c,COUNTER_MAX-1,0,false,false);for(unsigned i=0;i<31000;i++)counter_sample(&c,true,false,i);
    ...
    counter_init(&c,5,0,false,false);counter_sample(&c,false,false,30001);assert(!c.awake);
    ...
    key_semantics();
    /* Actual queue + app while every storage word takes worst-case8ms. */
    for(unsigned latency=0;latency<=8;latency++)for(unsigned stride=1;stride<INPUT_QUEUE_SIZE;stride++) {
        Memory m={0};m.latency=latency;record(m.words,0,0,false);NvHal h=hal(&m);App a;app_init(&a,&h,0,0);InputQueue q={0};uint32_t t=0;
        for(;t<1600;t++) {
    ...
    /* The power-on press counts once and is persisted; not while ERR. */
    {Memory m={0};record(m.words,3,41,false);NvHal h=hal(&m);App a;app_init(&a,&h,0,INPUT_INCREMENT);app_wake_press(&a,0);
     uint32_t t=1;for(unsigned i=0;i<100;i++)sample(&a,INPUT_INCREMENT,t++);app_finish(&a,&m,&t);assert(a.journal.count==42);
     Memory blank={0};NvHal bh=hal(&blank);App b;app_init(&b,&bh,0,INPUT_INCREMENT);app_wake_press(&b,0);assert(b.counter.count==0&&app_fault(&b));}
}
static void fault_recovery(void){
    ...
    for(unsigned stop=0;stop<200;stop++) {
        Memory m={0};m.latency=8;record(m.words,7,55,true);NvHal h=hal(&m);App a;app_init(&a,&h,0,0);uint32_t t=0;
        for(unsigned i=0;i<RESET_HOLD_MS+20;i++)sample(&a,INPUT_RESET,t++);
        assert(!a.input.reset_requested&&a.counter.count==55&&a.counter.reset_armed); /* armed, not committed */
        ...
        hold_reset(&a,&t,RESET_HOLD_MS+20,0);
        ...
        for(unsigned n=0;n<10;n++)sample(&a,0,t++);
        for(unsigned n=0;n<10;n++)sample(&a,INPUT_INCREMENT,t++);
        assert(a.counter.count==2);++interleavings;
    }
    /* A gap while the reset key is held adopts it as a level: its release
     * cannot clear ERR; only a new complete hold does. */
    {Memory m={0};record(m.words,7,55,false);NvHal h=hal(&m);App a;app_init(&a,&h,0,0);uint32_t t=0;
     for(unsigned i=0;i<1000;i++)sample(&a,INPUT_RESET,t++);
     InputSample gap={t,INPUT_RESET};app_input(&a,INPUT_GAP,&gap);
     for(unsigned i=0;i<3000;i++)sample(&a,INPUT_RESET,t++);for(unsigned i=0;i<20;i++)sample(&a,0,t++);
     assert(app_fault(&a)&&!a.input.reset_requested&&a.counter.count==55);
     hold_reset(&a,&t,RESET_HOLD_MS+20,0);assert(a.input.reset_requested&&a.counter.count==0);app_finish(&a,&m,&t);assert(!app_fault(&a));}
    for(unsigned failure=0;failure<3;failure++){
        Memory m={0};m.latency=8;record(m.words,0,0,false);NvHal h=hal(&m);App a;app_init(&a,&h,0,0);uint32_t t=0;app_finish(&a,&m,&t);
...
static void fault_idle(void){
     ...
     * follows a NEW complete reset; INC wake and a stuck reset stay faulted. */
    for(unsigned fault=0;fault<4;fault++)for(unsigned wrap=0;wrap<2;wrap++){
        ...
        for(unsigned i=0;i<20;i++){sample(&a,0,t);app_service(&a,&h,t++,true);}
        assert(!a.input.reset_requested); /* the abandoned hold is not a reset */
    ...
    Memory m={0};record(m.words,1,4,false);NvHal h=hal(&m);App a;app_init(&a,&h,0,0);
...
static void crc_equivalence(void){
    /* Nibble-table CRC must equal the bitwise reference for arbitrary words. */
    uint32_t x=0x12345678UL;
    for(unsigned n=0;n<20000;n++){
        uint32_t w[6];for(unsigned i=0;i<6;i++){x^=x<<13;x^=x>>17;x^=x<<5;w[i]=x;}
        uint32_t crc=0xffffffffUL;
        for(unsigned i=0;i<6;i++)for(unsigned b=0;b<4;b++){crc^=(w[i]>>(b*8))&255U;for(unsigned bit=0;bit<8;bit++)crc=(crc>>1)^((crc&1U)?0xedb88320UL:0);}
        assert(journal_crc(w,6)==~crc);
    }
}
int main(void){crc_equivalence();journal_tears();behavior();fault_recovery();fault_idle();printf("{\"key_hold_cases\":%lu,\"torn_word_reboots\":%lu,\"single_bit_corruptions\":%lu,\"button_waveforms\":%lu,\"queue_storage_schedules\":%lu,\"recovery_interleavings\":%lu,\"fault_idle_recovery_scenarios\":%lu,\"hardware_executed\":false}\n",key_cases,tears,corruptions,waves,queue_schedules,interleavings,fault_idles);return 0;}
