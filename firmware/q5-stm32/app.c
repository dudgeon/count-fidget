#include "app.h"
#include <string.h>
void app_init(App *a,const NvHal *h,uint32_t now,uint8_t raw) {
    memset(a,0,sizeof(*a));journal_load(&a->journal,h);
    counter_init(&a->counter,a->journal.count,now,(raw&INPUT_INCREMENT)!=0,(raw&INPUT_RESET)!=0);
...
void app_wake_press(App *a,uint32_t now) {
    if(!app_fault(a))(void)counter_wake_press(&a->counter,now);
    else {a->counter.awake=true;a->counter.last_activity=now;}
}
