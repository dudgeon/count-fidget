...
static void power(bool on) {
    ...
    if(!on && powered && !reset_low)assert(((uint32_t)(time_ms-last_disable_ack)-1U)*1000U>=103500U);
    powered=on;
}
    ...
    if(queued[0]==0x40) {
        ...
        for(unsigned i=1;i<length;i++) {
            ...
            if(command==0x8d) {
                ...
                assert(argument==0x10 || argument==0x72);
                pump_on=argument==0x72;
...
static void await_render(Oled *d,uint32_t count) {
    ...
    while(!(d->state==OLED_READY && d->displayed_valid && d->displayed_count==count && d->displayed_fault==input_fault)) {
        step(d,true,count);assert(++bounded<4000);
    }
    for(unsigned p=0;p<4;p++)for(unsigned x=0;x<128;x++)
        assert(ram[p][x]==oled_status_byte(count,input_fault,(uint8_t)p,(uint8_t)x));
}
typedef struct {Record slots[2];unsigned commits;} Store;
static bool store_word(void *context,unsigned slot,unsigned word,uint16_t value) {
    Store *s=context;s->slots[slot].words[word]=value;
    if(word==7 && value)++s->commits;
    return true;
}
int main(void) {
    ...
    assert(oled_pixel_byte(COUNTER_MAX,3,58)==0x7c);
    assert(oled_pixel_byte(123,3,58)==0);
    assert(oled_pixel_byte(0,4,0)==0 && oled_pixel_byte(0,0,128)==0);
    input_fault=true;await_render(&d,COUNTER_MAX);
    assert(d.displayed_fault && oled_status_byte(COUNTER_MAX,true,3,59)==0x54);
    input_fault=false;await_render(&d,COUNTER_MAX);
    transfer_delay=19; /* longest successful modeled transfer before timeout */
    unsigned limit=0;
    while(!oled_is_off(&d)){step(&d,false,COUNTER_MAX);assert(++limit<300);}
    assert(!powered && reset_low && !panel_on && !pump_on);
    assert(((uint32_t)(time_ms-last_disable_ack)-1U)*1000U>=103500U);
    puts("PASS: SSD1312 startup/clear/9V commands, bounded chunks, dirty/coalesced frames, MAX, shutdown guard");

    fresh(&d,0);await_render(&d,77);
    fail_on_transfer=transfers+4;await_render(&d,88);
    assert(d.errors==1 && failures==1);
    puts("PASS: injected mid-frame NACK discards display state and recovers the latest committed count");

    /* Counter sampling and journal commits continue through repeated bus
     * timeouts, reset guard and backoff, using the actual display engine. */
    fresh(&d,0);always_fail=true;Counter c;counter_init(&c,0,0);
    Store store;memset(&store,0xff,sizeof store);store.commits=0;
    uint32_t committed=0;
    for(unsigned n=0;n<6000;n++) {
        bool press=(n%40U)>=5 && (n%40U)<20;
        counter_sample(&c,press,false,time_ms);
        if(c.dirty){assert(journal_save(store.slots,c.count,store_word,&store));committed=c.count;c.dirty=false;}
        step(&d,c.awake,committed);
    }
    assert(c.count==150 && store.commits==150 && d.errors>3 && failures>3);
    uint32_t saved,sequence;unsigned slot;
    assert(journal_load(store.slots,&saved,&sequence,&slot) && saved==150);
    always_fail=false;await_render(&d,committed);
    puts("PASS: 150 sampled/committed presses through absent-display timeouts, bounded retries and recovery");

    fresh(&d,0xfffffff0UL);await_render(&d,1);
    while(!oled_is_off(&d))step(&d,false,1);
    assert(!powered);
    for(unsigned cut=0;cut<30;cut++) {
        fresh(&d,0);
        for(unsigned n=0;n<cut;n++)step(&d,true,0);
        /* Sleep before reset release needs no I2C command to an unready IC. */
        limit=0;while(!oled_is_off(&d)){step(&d,false,0);assert(++limit<300);}
        assert(!powered);
    }
